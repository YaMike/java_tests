...
public class Fast {
  ...
  private static void processCollinearPoints(Point p, Point[] points, int start, int count) {
    ...
    Arrays.sort(parr);                                                  if (DEBUG) { StdOut.printf("Origin point =%s, parr[0]=%s, compare_res = %d\n", p, parr[count], parr[0].compareTo(p)); }
  ...
  public static void main (String[] args) {
    ...
    Arrays.sort(origins);                                             if (DEBUG) { printPointsArray("origins", origins); }

    for (int i = 0; i < origins.length; i++) {                        if (DEBUG) { StdOut.printf("\n*************************\n"); }
      p = origins[i];                                                 if (DEBUG) { StdOut.printf("i = %d\norigin point = %s\n", i, p); }
      Point[] points = new Point[origins.length];
      System.arraycopy(origins, 0, points, 0, origins.length);        if (DEBUG) { printPointsArray("Origins copy", points); }
      Arrays.sort(points, p.SLOPE_ORDER);                             if (DEBUG) { printPointsArray("Sorted origins copy", points); }

      int slopesCount = 0, start = 0;
      if (i > 0 && p.compareTo(origins[i-1])==0) continue;

      for (int j = 0; j < points.length; j++) {
        q = points[j];
        pqSlope = p.slopeTo(q);
        if (j == 0) {
          prevSlope = pqSlope;                                        if (DEBUG) { StdOut.printf("Slope=%f,%s,j=%d,col=%s,slopes cnt=%d\n", pqSlope, q, j, "unknown", slopesCount); }
          continue;
        }
